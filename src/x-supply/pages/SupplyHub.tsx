...
// 权限：EU/VDM/DU 可进；XU/CU 前端守卫 403 + 后端接口 403 双保险
// 办位（EX/EXX）：入驻登记 + Booth-E 铺面维护（仅本铺）；EU 管位只读 + 指引；VDM/DU 治理/采购视角
      ...
      <div className="mb-5 rounded border border-[#e4ded2] bg-white px-4 py-3 text-sm text-[#57534e] shadow-[4px_4px_0_rgba(23,24,29,0.08)]">
        {isExec ? (
          ...
          <span className="flex flex-wrap items-center gap-2">
            ...
            治位视角（四源分线 · X-MARKET-ROLE-01）：{supplyGovernDomainOf(hat) ? `${hat} 治 ${supplyGovernDomainOf(hat)} 源准入与货品` : 'VDM 统筹全域四源（含大额采购审批/阈值）'}；下方治理台办理。
          </span>
        ) : hat === 'DU' ? (
          <span className="flex flex-wrap items-center gap-2">
