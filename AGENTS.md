...
## XMK-GOV-01 E-Market 治理面+供集集中管理（2026-09-11，VEM 打样）

- **治理分权定版**：市管方=XVPZ 平台方分治制，E 域打样——`terminology.ts` v1.5 `MARKET_GOVERNORS`（vem 条目：big=市管方/sys=XVPZ#VEM/plat=X-Goods/market=E-Market/定位语；其余 Plat 未裁定不开治理口）+`governorTermOf(platKey)`+CONCEPT_TERMS.governanceDesk{big:市管台,sys:XVPZ · 治理面}。
- **身份**：第 48 演示账号 `vem-1`（XVPZ#VEM，domainView T，note 注明 T-PLAT 平台域；末位追加）——新容器 `c-xvp1`（type XVPZ）+unit `u-vem2`（VEM）；旧 VEM 账号 `vem-e` 挂 c-plat（XOPZ）→新治理面 403（准入=hatRole VEM 且 containerType XVPZ 双条件）。
- **治理 API（新命名空间 /api/governance，`server/governance/routes.ts`，既有 /api/customer、/api/supply 契约零触碰）**：GET `supply/vendors`（E 域入驻名录=Booth-E 主体+vendor_status+治理事件）、POST `supply/vendors/:id/audit`（body {action:approve|freeze,note}——状态机 pending→approved→frozen→approved；非法迁移 **409**、未知 action 400）、GET `supply/orders`（全域只读+status stats 聚合）、GET `overview`（E-Market 看板：volume/amount/approved_vendors/status_dist）。准入矩阵：仅 XVPZ#VEM 200；VDM/CU/DU/EU/旧 VEM 403；匿名 401。
- **Vendor 准入状态（`server/x-supply/store.ts`）**：XSupplyProfile 扩 `vendor_status`（pending|approved|frozen，VendorStatus）+`vendor_note`+`governance_events`（snake_case：action/actor_user/actor_hat=VEM/target/note/ts）；seed c-qc/c-eu01/c-dp1=approved、**c-ep2（新恒达物资，XEPZ 新容器+u-ep2-eu）=pending**（无演示账号，名录状态迁移用）。
- **供集联动拦截（XMK-GOV-01）**：`x-supply/routes.ts` POST /orders 前置——buyer 容器或 supplier 容器（booth.ownerUnitId→unit.containerId）vendor_status==='frozen'→403「入驻主体已被市场治理冻结（VEM）」；audit 端点同步写 xSupplyProfiles（真状态变更，禁 mock）。
- **治理台 UI（/governance，`src/pages/GovernanceDesk.tsx`）**：页面级守卫（VEM+XVPZ else 拒绝页，RoleGuard 不动）；四区块=入驻审核队列（批准/冻结+note）/供给单监察表/供给方名录（含治理事件时间线）/E-Market 看板；紫 govern 风格+DualTerm 术语常量零硬编码；`src/api/governance.ts`（governanceApi.overview/vendors/audit/orders）；Header governSupply nav 增「市管台」（conceptTerm('governanceDesk').big）。
- **回归（全绿）**：gov01-check **38/38**（矩阵逐格+freeze→supply 403→approve→200 闭环+c-ep2 pending→approve→409 重复 approve→freeze→409→400 未知 action+事件七要素）；cont01-check **10/10**（oneclick **48/48**）；booth-timeline-ui **10/10**；test_run lint+ts-check+governance smoke PASS。历史全量 lint 28 errors 为遗留 unused（本轮新文件零 error，交付以 test_run 增量为准）。

## XMK-CRM-UI-01 客集工作台 UI（2026-09-11，X-Customer 客户集市 F1-F5 全量）

- **F1 客集工作台（/customer，`src/pages/CustomerWorkbench.tsx`，公开路由+页面级守卫，RoleGuard 不动）**：四区块纵向（客蓝 #1D4ED8）——F2 需求单（title 必填+desc 发布表单+列表卡 code/title/desc/状态徽标/时间）+F3 采购意向（同构+「关联需求单」select=本地展示层关联不落库，选中卡上「已关联 XCD-xxx」）+F4 客户档案（只读卡 identity_id/container_name/container_type/hat_role/display_name+顶部声明「档案由 OAS 派生，不可编辑——本页为零编辑入口」+区块内零 button）+F5 供给单时间线（`req<XSupplyOrder[]>('/api/supply/orders')` buyer 视角：状态徽标+时间线 events+confirm 按钮仅 quoted 态+旁常显知情文案「责任转移+触发结算」+重复 confirm 409 红条）。术语全走 terminology 常量：**v1.5 客集批次词条** customerWorkbench{客集工作台/X-Customer · 客户集市}/customerDemand{需求单/XCD}/customerIntent{采购意向/XCI}/customerProfile{客户档案/CRM 档案 · OAS 派生只读}/customerTimeline{供给单时间线/X-Supply · buyer 视角}。
- **身份矩阵 UI 守卫（客户端四分支）**：匿名→useEffect navigate('/entrance')；XVPZ 系（hatRole==='VEM'||containerType==='XVPZ'）→引导页「平台方治理身份请前往治理台」+Link /governance；非 CU（XDPZ#DU 等）→拒绝页「准入不通过」+「返回我的工作台」；CU（XHPZ#CU 与 XEPZ#CU 均客集写）→全功能面。**三层导航客集区入口条**：Mall.tsx MarketLayerNav 紧后挂 Link /customer（Users icon+customerWorkbench.big+「需求单 / 采购意向 / 客户档案」摘要）。
- **E2E 注意（ENTRANCE-01 一角色一登入）**：直写 localStorage xm_token+选帽 xm_role 后 goto 业务路由仍会被 `EntranceGate`（user&&!activeRole→/entrance/role）拦截——E2E 必须**UI 完整过门**：goto /entrance→点角色卡按钮→waitText 工作台特征文本→再 goto 目标路由。crm-ui-check.mjs `uiLogin(demoId, capKey, waitText)` 已封装（playwright-core+executablePath 显式）。
- **回归（全绿）**：新增 crm-ui-check **27/27**（F1 匿名跳登录/XHPZ#CU 三区块+档案只读零编辑/F2 空标题 400+发布即时可见/F3 意向+关联展示/F4 identity_id/F5 构单 initiate→accept→quote→UI confirm→已确认+API 重复 confirm 409+知情文案/DU 拒绝页/VEM 引导页，四截图 assets/crm01/）；六套零破 embed-e2e 8/8+embed-diag 12/12+exchange 7/7+api01 33/33+gov01 38/38+cont01 10/10（48/48）；截图基线 cont01-shots 17/17+struct01-shots 7/7；test_run lint+ts-check+客集 smoke（demands POST 落库 XCD-2026-0009+profile 派生）PASS。

## XMK-MALL-RICH-01 集市内容扩容（2026-09-11，34 SKU+/mall 逛购三件套/客集动线/goods 同源只读）

- **数据（真实落库 store.listings）**：`shared/types.ts` Listing 增量可选字段 category（'food'|'grain'|'specialty'|'daily'）/desc/stock/img/supplyBoothId（不破既有模型）；`server/store.ts` 新增 **b-dc2「知味·风物直营店」**（u-dcx2 执行帽，同 c-du 经营容器）+32 SKU seed（l101-l132：food 10/grain 7/specialty 8/daily 7，对齐 b-dc1 12+b-dc2 20 两铺）+存量 l15/l16 补 category/desc/stock/img——**共 34 件全字段**（GET /mall/listings `{...l,booth}` 自动透传零改动）。图片=**本地 SVG 资产 34 张**（`scripts/gen-mall-imgs.mjs` 品类渐变+首字生成→public/img/mall/sku-N.svg，禁外链，SVG 内容无远程引用）。
- **/mall 逛购升级（`src/pages/Mall.tsx`）**：逛购三件套——品类筛选条（全部/食品生鲜/粮油调味/地方特产/日用百货，ALL_CATS 常量+CAT_META 品类色）/关键词搜索（title/tags/desc 包含，空关键词全量）/排序 select（默认/价格从低到高/从高到低）+**商品详情弹层**（role=dialog：大图+desc/stock/supplyBoothId/booth+按钮组「立即购买/发起采购意向/提交需求单/企业采购」）。动线：openConfirm 前 setDetail(null) 防遮罩叠层；needLogin 弹层复用（意向/需求/企业采购未登录同样引导，「去登入端」Link **不得带 onClick setState**——React Router Link onClick 内 setState 卸载自身会打断 navigation）；CU 意向/需求→customerApi.post*（成功回执含「查看客集工作台」Link /customer+即时 reload 供集时间线）；「企业采购」→`req<XSupplyOrder>('/api/supply/orders',{method:'POST',body:{boothId:l.supplyBoothId,title,qty,unit}})`——XEPZ#CU（cu-ep1）200 回执供集单 code；XHPZ#CU/DU 走服务端 403→err 红条不静默（身份矩阵不变，服务端强制）。图片懒加载 loading="lazy"（首屏外）。
- **/goods 同源只读（`src/pages/Goods.tsx`）**：同源 api.mallListings（全量不过滤域）只读网格（图/名/价/品类标+「E-Market 通货集市 · 只读浏览」标注）+「采购意向」入口（CU→postIntent；未登录→navigate('/entrance') 引导不静默）+**零「立即购买」**（B 端完整采购流程后续工单）。
- **回归（全绿）**：新增 mall-check **22/22**（T1 catalog 34 件四品类+全字段/T2 本地 SVG 4 抽 200/T3 匿名三件套+详情弹层/T4 匿名两动线引导+Link 跳登入端/T5 XHPZ#CU 意向回执+需求单回执+供集 403 红条/T6 XEPZ#CU 供集 buyer 成功/T7 DU 意向 403 红条/T8 goods 同源≥32+只读+意向引导，7 截图 assets/mall01/）；六套零破 embed-e2e 8/8+embed-diag 12/12+exchange 7/7+api01 33/33+gov01 38/38+cont01 10/10（48/48）；**crm-ui-check 27/27 零破**；test_run lint+ts-check+mall smoke（34 SKU/品类分布/all_img/all_desc+SVG 200）PASS。
- **E2E 经验**：test_run pipeline 对每条 curl 自动追加 `-w '\nHTTP_CODE:%{http_code}'`（第二行）——管道 python 解析 JSON 必须 `json.loads(sys.stdin.readline())` 只读首行；**并发跑两个 playwright 脚本会瞬时资源冲突**（chromium 启动崩溃）——回归脚本串行执行；弹层叠层断言前先关上层（Esc/「先逛逛」）。

## XMK-EU-CHAIN-01 供给链收口（2026-09-11，准入审批闭环+EU 上架链+goods B 端采购+品牌口径）

- **A 线·准入审批端到端闭环（P0）**：SupplierApplication.status 扩 `'submitted'|'reviewing'`（shared/types），rejected 可重提。**治理三端点（`server/governance/routes.ts`，/api/governance 命名空间）**：GET `applications`（服务端拍平 `{...a, containerName}`，createdAt 逆序）/POST `applications/:id/claim`（submitted|pending→reviewing+reviewedBy，重复 claim **409**）/POST `applications/:id/audit`（body {action:approve|reject,note}——非 reviewing **409**；approve→profile 回写 vendor_status='approved'（XSupplyProfile 不存在先建骨架 contact_name:''/contact_phone:''/intro:''）；reject→'rejected'+rejectReason 同步 profile vendor_note；治理事件 action 扩 `application_approve|application_reject`，actor_hat=user.hatRole as HatRole）。**审批主体**：requireGovernor 扩 `['VEM','VXM']`（X-MARKET-08 口径 VXM=评估统筹+GOV-01 治理体系对齐，**不新造帽**——HatRole 补 'VXM'+UNIT_ROLE_LABEL '云统筹审批'+HAT_LINE_OF 'admin'+HAT_POWER_BITS ['govern']，三处 Record 完备性连锁必补）。**重提链**：POST /api/supply/applications 新建 status='submitted'；rejected 重提→submitted（resubmitCount 递增）。回写后 GET /api/supply/profile 可见，ZiwayOS SupplyHub 自动展示。
- **B 线·EU 上架卖货链（P0）**：POST /api/supply/products 必填 `name`（非 title）/category/priceCents——前置链：product_publish 权位→isSupplyHat→**准入 approved**（403「仅合格供应商」）→mySupplyBooth（404 无供给铺）——**A1 联动**：新供给帽须先开供给实体铺（POST /api/market/booths {domain,kind:'supply',name}，booth_new allow 含 EU）才能提交准入。GET /api/supply/products **守卫修复（ROLE-01 收敛的回归损伤）**：治理帽域过滤分支+**供给帽本人 own 分支**（supplierId===user.containerId，SupplyDesk 上架管理依赖）+其余 403。SupplierProduct 下架字段=`status:'on'|'off'`（**非 listed**——断言勿踩）。DU 承接=GET /api/supply/mall（含供给方名）→POST /api/orders {supplierProductId,qty}。治理下架 take-down（家族帽/E 源）照旧。
- **C 线·goods B 端企业采购（P1）**：`src/pages/Goods.tsx` 企业采购专区（api.supplyMall() catch 403→红条不静默；下单 api.createOrder({supplierProductId,qty})→**EX-2026-xxxx 族码**回执展示）——C/B 双轨并存（主人拍板都开放）。矩阵：XDPZ#DU 200 族码单；XHPZ#CU **403**（企业采购=B 端 DU 专属，供集 XS-单才是 CU 口）；匿名 **401**。
- **D 线·品牌口径**：露出「百泰OS · 知味数智生态驱动」footer（Mall.tsx+Goods.tsx，Boxes icon）——**Mall footer 禁用 paper-card 类**（会污染 mall-check `.paper-card` 卡片计数致 T3c 假破）；bybt/百业百泰零出现（check D1/D2 断言）。
- **demo 账号（48→50）**：`eu-9x`（恒泰建材 c-eu9x/u-eu9x，approve 线）/`eu-10x`（沃野农产 c-eu10x/u-eu10x，reject→重提线）——**python 全局 replace 污染教训**：改名波及既有 u-eu9h/u-eu10h/c-eu10 且 demo.containerId 未跟改（**SessionUser.containerId 取 demo 记录非 unit**）→脏申请数据；修复后 grep 验证唯一性。
- **回归（全绿）**：新增 eu-chain-check **30/30**（A0 开铺+A1 提交+A1b CU 403+A1c 供给帽调治理端点 403+A2 队列+A3 claim/409+A4 approve+A5 profile 回写+A6-A9 reject→重提链+B1 上架/B1b 未合格拒/B2 承接/B3-B3b 治理下架+C1 DU 族码单/C2 CU 403/C3 匿名 401+U1-U3b UI+D 品牌断言，截图 assets/euchain01/）；六套 cont01 10/10（50/50）/gov01 38/38/api01 33/33/booth-timeline（**脚本修复**：文案对齐「履约时间线/Booth · 履约四节点」实态+深链选择器 `a[href*="fulfillment-track"]`（组件无 data-booth-deeplink）+清零态空态口径「暂未进入履约」为合法态）/booth-timeline-ui 10/10/ux01 4/4；mall-check **22/22**（footer paper-card 修复后）；crm-ui-check **27/27**；test_run lint+ts-check+探活 PASS。
- **E2E 经验**：**A 线状态机测试依赖清零态**——重跑前必须重启服务（ss 取 pid kill，pkill 'tsx watch' 不命中），A1 撞 approved 单会连锁 FAIL；GET /supply/products 这类「收敛后守卫」改动要回归**供给帽本人**视角（ROLE-01 曾漏）；前端搜索类 UI 断言先 grep 种子唯一性+排查 footer 等非卡片元素类名污染。
