// X-MARKET-EMBED-01（EMBED-01-FIX）· /api/embed/exchange 联调自测（单命令内串行，规避 test_run 乱序并行）
// 前置：mock verify 已在 :9301（scripts/embed-verify-mock.mjs），dev server 已以
//       ZIWAY_EMBED_BASE=http://127.0.0.1:9301 启动。
// 断言七态：包装格式合法票 200 会话 / 裸格式兼容票 200 会话 / 同票重放 401 /
//           未知角色 403 / realm 不匹配 403 / app 不匹配 401 / 票格式非法 400。
...
// 1) 包装格式合法票（真实 ZiwayOS {code,data} 形态）：200 + xm_ token + CU 消费态会话
const t1 = `zt_ok_${stamp}`;
const r1 = await ex(t1);
const d1 = r1.j?.data; // ok() 响应外壳 {success, data}
assert('valid-ticket-wrap-200', r1.status === 200 && typeof d1?.token === 'string' && d1.token.startsWith('xm_') && d1.user?.hatRole === 'CU' && d1.embed?.role === 'CU',
  `status=${r1.status} role=${d1?.user?.hatRole} embed=${JSON.stringify(d1?.embed)} token=${d1?.token ? `${d1.token.slice(0, 3)}…` : 'none'}`);

// 2) 裸格式兼容票（{ok:true} 顶层，mock 旧形态）：200 会话
const r1b = await ex(`zt_bare_${stamp}`);
const d1b = r1b.j?.data;
assert('valid-ticket-bare-200', r1b.status === 200 && typeof d1b?.token === 'string' && d1b.token.startsWith('xm_') && d1b.user?.hatRole === 'CU',
  `status=${r1b.status} role=${d1b?.user?.hatRole} token=${d1b?.token ? `${d1b.token.slice(0, 3)}…` : 'none'}`);

// 3) 同票重放：401 + ticket_replayed（票单次消费，由 ZiwayOS verify 负责；message 透出为 reason）
const r2 = await ex(t1);
assert('replay-401', r2.status === 401 && r2.j?.reason === 'ticket_replayed', `status=${r2.status} reason=${r2.j?.reason}`);

// 4) 未知角色票（verify ok 但 role=DU）：403 非 CU/GU
...
// 5) realm 不匹配票（verify ok 但 realm=other）：403
...
// 6) app 不匹配票（verify 拒绝 app_mismatch，message 透出）：401
