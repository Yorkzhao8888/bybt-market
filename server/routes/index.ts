...
const demoAccounts: DemoAccount[] = [
  ...
  // 总经营管理执行 VDM（X-MARKET-18 v1.2 终版：VXM→VDM→VDM 合并，归 OVM；域内管家审批仍 V*M 系 + 大额采购审批统筹）
  { id: 'vdm-cloud', entry: 'B', hatRole: 'VDM', containerId: 'c-plat', hatId: 'u-vxm1', label: '总经营管理执行 VDM · 云统筹', note: '总经营管理执行（归 OVM）+ supply 管家审批全域统筹（域内准入评估/货品审批/大额采购审批/阈值配置）', domainView: 'E' },
...
const DEMO_ROUTE: Partial<Record<HatRole, DemoAccount>> = {
  ...
  DU: demoAccounts[7], XU: demoAccounts[9], VDM: demoAccounts[10], VEM: demoAccounts[11], EX: demoAccounts[13],
...
api.get('/orders', requireAuth, (req: AuthReq, res) => {
  ...
  if (can(user, 'view_all_orders')) {
    // X-MARKET-ROLE-01 治理分线：VDM（经营治理）见全局总账；V*M 四源家族（VDM/VEM/VHM/VYM/VTM）
...
api.get('/supply/applications', requireAuth, (req: AuthReq, res) => {
  const user = req.user!;
  const role = roleOf(user);
  // X-MARKET-ROLE-01 治理分线：审核列表归 supply 四源治理（VDM 全域 + 家族本源域过滤）；VDM（market 经营治理）不越界
...
// 四源治理：审核（通过→合格；驳回→附原因，供给方可重提）（X-MARKET-ROLE-01：VDM 全域 + 家族本源域分线）
api.post('/supply/applications/:id/review', requireAuth, (req: AuthReq, res) => {
  ...
  // 四源分线：家族帽仅可裁决本源域申请（VEM→E/VYM→Y/VHM→H/VTM→T；VDM 全域）
  const govDomain = supplyGovernDomainOf(role);
...
// 四源治理：货品治理列表（违规下架用，客户不可见）（X-MARKET-ROLE-01：VDM 全域 + 家族本源域过滤；VDM 不越界）
...
api.post('/supply/products', requireAuth, (req: AuthReq, res) => {
  ...
  if (!app || app.status !== 'approved') {
    res.status(403).json({ success: false, error: '仅合格供应商可上架货品（先通过 VDM 云中心准入评估）' });
    return;
  }
  const booth = mySupplyBooth(user);
  if (!booth) {
...
// 上架/下架：供给方本人切换；VDM 可治理下架违规货品（重新上架须由供给方操作）
api.post('/supply/products/:id/toggle', requireAuth, (req: AuthReq, res) => {
  ...
  if (owner) {
    ...
    // X-MARKET-ROLE-01 四源分线：家族帽仅可治理本源域货品（VEM→E/VYM→Y/VHM→H/VTM→T；VDM 全域）
    const govDomain = supplyGovernDomainOf(role);
...
api.post('/supply/products/:id/take-down', requireAuth, (req: AuthReq, res) => {
  ...
  if (!checkPower('product_govern_remove', req, res, boothCodeOf(prod.boothId))) return;
  // X-MARKET-ROLE-01 四源分线：家族帽仅可治理下架本源域货品（VDM 全域）
...
api.get('/power/audit', requireAuth, (req: AuthReq, res) => {
  ...
  // 治位帽（V*M/VDM）见全量审计；其余身份仅见本人（actor_user）动作留痕（X-MARKET-13：本人口径，含 denied 越权尝试）
...
api.get('/power/dashboard', requireAuth, (req: AuthReq, res) => {
  ...
  const isGovern = (HAT_POWER_BITS[hat] ?? []).includes('govern');
  if (!isGovern) {
    res.status(403).json({ success: false, error: `三权运行看板属治位（云审批评估）查询，帽 ${hat} 无此权（需 VDM/VEM/VDM）` });
    return;
  }
  const now = Date.now();
